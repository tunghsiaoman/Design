...
public class ImageSizer
{
	...
	public static void resizeWH(File originalFile, File resizedFile, int width, String format, int height) throws IOException
	{
		if (format != null && "gif".equals(format.toLowerCase()))
		{
	...
	private static void resizeH(File originalFile, File resizedFile, int newWidth, float quality, int height) throws IOException
	{
	 ...
	 * @throws IOException
	 */
	public static void resizeCrop(File originalFile, File cropFile, int maxWidth, float quality) throws IOException
	{
		if (quality < 0 || quality > 1)
		{
			throw new IllegalArgumentException("Quality has to be between 0 and 1");
		...
		int MAX_IMAGE_SIZE = maxWidth;
		if (iWidth > iHeight)
		{
			resizedImage = i.getScaledInstance((int) ((float) iWidth / (float) iHeight * MAX_IMAGE_SIZE), MAX_IMAGE_SIZE, Image.SCALE_SMOOTH);
		} else
		{
			resizedImage = i.getScaledInstance(MAX_IMAGE_SIZE, (int) ((float) iHeight / (float) iWidth * MAX_IMAGE_SIZE), Image.SCALE_SMOOTH);
		}
		...
		if (bWidth > bHeight)
		{
			croppedImage = bufferedImage.getSubimage((bWidth - bHeight) / 2, 0, bHeight, bHeight);
		} else
		{
			croppedImage = bufferedImage.getSubimage(0, (bHeight - bWidth) / 2, bWidth, bWidth);
		}
		...
		Kernel kernel = new Kernel(3, 3, softenArray);
		ConvolveOp cOp = new ConvolveOp(kernel, ConvolveOp.EDGE_NO_OP, null);
		croppedImage = cOp.filter(croppedImage, null);
		// Write the jpeg to a file.
